/*
 *  IBM Confidential OCO Source Materials
 *                                                                 
 *  5725-I43 Copyright IBM Corp. 2011, 2017
 *                                                                   
 *  The source code for this program is not published or otherwise
 *  divested of its trade secrets, irrespective of what has
 *  been deposited with the U.S. Copyright Office.
 *                   
 */


/*
 * Tone Analyzer
 * ### Service Overview The IBM Watson&trade; Tone Analyzer service uses linguistic analysis to detect emotional, social, and language tones in written text. The service can analyze tone at both the document and sentence levels. You can use the service to understand how your written communications are perceived and then to improve the tone of your communications. Businesses can use the service to learn the tone of their customers' communications and to respond to each customer appropriately, or to understand and improve their customer conversations. ### API Usage The following information provides details about using the service to analyze tone: * **The tone method:** The service offers `GET` and `POST /v3/tone` methods that use the general purpose endpoint to analyze the tone of input content. The methods accept a maximum of 128 KB of content in JSON, plain text, or HTML format. * **The tone_chat method:** The service offers a `POST /v3/tone_chat` method that uses the customer engagement endpoint to analyze the tone of customer service and customer support conversations. The method accepts a maximum of 128 KB of content in JSON format. * **Authentication:** You authenticate to the service by using your service credentials. You can use your credentials to authenticate via a proxy server that resides in Bluemix, or you can use your credentials to obtain a token and contact the service directly. See [Service credentials for Watson services](https://console.bluemix.net/docs/services/watson/getting-started-credentials.html) and [Tokens for authentication](https://console.bluemix.net/docs/services/watson/getting-started-tokens.html). * **Request Logging:** By default, all Watson services log requests and their results. Data is collected only to improve the Watson services. If you do not want to share your data, set the header parameter `X-Watson-Learning-Opt-Out` to `true` for each request. Data is collected for any request that omits this header. See [Controlling request logging for Watson services](https://console.bluemix.net/docs/services/watson/getting-started-logging.html).   For more information about the service, see [About Tone Analyzer](https://console.bluemix.net/docs/services/tone-analyzer/index.html).
 *
 * OpenAPI spec version: 3.3.4
 * 
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

package com.ibm.mfp.adapters.sample.model;

import java.util.Objects;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.ibm.mfp.adapters.sample.model.ToneCategory;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import java.util.ArrayList;
import java.util.List;
import javax.validation.constraints.*;

/*
 *  IBM Confidential OCO Source Materials
 *                                                                 
 *  5725-I43 Copyright IBM Corp. 2011, 2017
 *                                                                   
 *  The source code for this program is not published or otherwise
 *  divested of its trade secrets, irrespective of what has
 *  been deposited with the U.S. Copyright Office.
 *                   
 */


/**
 * SentenceAnalysis
 */
@javax.annotation.Generated(value = "com.github.mfpdev.adapters.swagger.codegen.MfpAdapterCodegen", date = "2017-10-03T17:26:51.394+05:30")
public class SentenceAnalysis   {
  @JsonProperty("sentence_id")
  private Integer sentenceId = null;

  @JsonProperty("text")
  private String text = null;

  @JsonProperty("input_from")
  private Integer inputFrom = null;

  @JsonProperty("input_to")
  private Integer inputTo = null;

  @JsonProperty("tone_categories")
  private List<ToneCategory> toneCategories = new ArrayList<ToneCategory>();

  public SentenceAnalysis sentenceId(Integer sentenceId) {
    this.sentenceId = sentenceId;
    return this;
  }

   /**
   * The unique identifier of a sentence of the input content. The first sentence has ID 0, and the ID of each subsequent sentence is incremented by one.
   * @return sentenceId
  **/
  @JsonProperty("sentence_id")
  @ApiModelProperty(required = true, value = "The unique identifier of a sentence of the input content. The first sentence has ID 0, and the ID of each subsequent sentence is incremented by one.")
  @NotNull
  public Integer getSentenceId() {
    return sentenceId;
  }

  public void setSentenceId(Integer sentenceId) {
    this.sentenceId = sentenceId;
  }

  public SentenceAnalysis text(String text) {
    this.text = text;
    return this;
  }

   /**
   * The text of the input sentence.
   * @return text
  **/
  @JsonProperty("text")
  @ApiModelProperty(required = true, value = "The text of the input sentence.")
  @NotNull
  public String getText() {
    return text;
  }

  public void setText(String text) {
    this.text = text;
  }

  public SentenceAnalysis inputFrom(Integer inputFrom) {
    this.inputFrom = inputFrom;
    return this;
  }

   /**
   * The offset of the first character of the sentence in the overall input content.
   * @return inputFrom
  **/
  @JsonProperty("input_from")
  @ApiModelProperty(required = true, value = "The offset of the first character of the sentence in the overall input content.")
  @NotNull
  public Integer getInputFrom() {
    return inputFrom;
  }

  public void setInputFrom(Integer inputFrom) {
    this.inputFrom = inputFrom;
  }

  public SentenceAnalysis inputTo(Integer inputTo) {
    this.inputTo = inputTo;
    return this;
  }

   /**
   * The offset of the last character of the sentence in the overall input content.
   * @return inputTo
  **/
  @JsonProperty("input_to")
  @ApiModelProperty(required = true, value = "The offset of the last character of the sentence in the overall input content.")
  @NotNull
  public Integer getInputTo() {
    return inputTo;
  }

  public void setInputTo(Integer inputTo) {
    this.inputTo = inputTo;
  }

  public SentenceAnalysis toneCategories(List<ToneCategory> toneCategories) {
    this.toneCategories = toneCategories;
    return this;
  }

  public SentenceAnalysis addToneCategoriesItem(ToneCategory toneCategoriesItem) {
    this.toneCategories.add(toneCategoriesItem);
    return this;
  }

   /**
   * An array of `ToneCategory` objects that provides the results for the tone analysis of the sentence. The service returns results only for the tones specified with the `tones` parameter of the request.
   * @return toneCategories
  **/
  @JsonProperty("tone_categories")
  @ApiModelProperty(required = true, value = "An array of `ToneCategory` objects that provides the results for the tone analysis of the sentence. The service returns results only for the tones specified with the `tones` parameter of the request.")
  @NotNull
  public List<ToneCategory> getToneCategories() {
    return toneCategories;
  }

  public void setToneCategories(List<ToneCategory> toneCategories) {
    this.toneCategories = toneCategories;
  }


  @Override
  public boolean equals(java.lang.Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    SentenceAnalysis sentenceAnalysis = (SentenceAnalysis) o;
    return Objects.equals(this.sentenceId, sentenceAnalysis.sentenceId) &&
        Objects.equals(this.text, sentenceAnalysis.text) &&
        Objects.equals(this.inputFrom, sentenceAnalysis.inputFrom) &&
        Objects.equals(this.inputTo, sentenceAnalysis.inputTo) &&
        Objects.equals(this.toneCategories, sentenceAnalysis.toneCategories);
  }

  @Override
  public int hashCode() {
    return Objects.hash(sentenceId, text, inputFrom, inputTo, toneCategories);
  }


  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class SentenceAnalysis {\n");
    
    sb.append("    sentenceId: ").append(toIndentedString(sentenceId)).append("\n");
    sb.append("    text: ").append(toIndentedString(text)).append("\n");
    sb.append("    inputFrom: ").append(toIndentedString(inputFrom)).append("\n");
    sb.append("    inputTo: ").append(toIndentedString(inputTo)).append("\n");
    sb.append("    toneCategories: ").append(toIndentedString(toneCategories)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }
}

